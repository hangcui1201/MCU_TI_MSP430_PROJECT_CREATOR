...
# print(os.old_project_dirpath.abspath(os.path.dirname(__file__) + '/' + '../..'))
...
class ProjectCreatorGUI():
	...
	def __init__(self, master):
		...
		# self.browse_button_state = "disabled"

		# COECSL Logo
		photo_path = os.path.abspath(os.path.dirname(__file__) + '/')
		photo_path = photo_path + "\\COECSL.gif"
		self.coecsl_logo = PhotoImage(file=photo_path)
	...
	def create_msp430_project(self):
		...
		if project_msp430 == "MSP430F2272":
			...
			project_to_create = CreateProjectF2272(project_dir)
			project_to_create.create_project()
			#self.master.quit()

		elif project_msp430 == "MSP430G2553":

			project_to_create = CreateProjectG2553(project_dir)
			project_to_create.create_project()
			#self.master.quit()

		else:
			print("Some impossible thing happens!")
